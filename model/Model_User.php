...
class Model_User extends Db
{
	...
	{
		return $this->selectQuery('select user_email from user where user_email=? ', array($userName));
	}

	function getByIdPass($userName, $pass)
	{
		$arr = array($userName, $pass);
		$data = $this->selectQuery('select user_email from user where user_email=? and user_password=?', $arr);
